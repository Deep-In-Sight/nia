...
from .eyes import device_dict, Eye
import json
import cv2
...
class Info():
    ...
    def __init__(self, device,
        ...
        if scen:
            if base_dir:
                self._dir = base_dir
                self.fn_full = glob(self._dir + f"*{self.device}_*_{scen}_???.json")[0]
            else:
    ...
    def get_eye(self, fn_json):
        anno = json.load(open(self._dir+fn_json, 'r'))
        eye = Eye(anno["Annotations"]["annotations"])
        return eye

    def load_vid(self, dtype='rgb'):
        vdir = self._vid_dir.replace("RGB", dtype.upper())
        fn_vid = self.fn_vid.replace("rgb", dtype)

        cap = cv2.VideoCapture(vdir+fn_vid)
        if cap.isOpened():
            return cap
        else:
            print("Can't open the vidio")
            print(vdir+fn_vid)
            return False
...
class Info_list():
    ...
    def __init__(self, 
        ...
        self.device_d = [device_dict[dd] for dd in self.devices]
        self.dtype = dtype
        self.status = status
        self.posture = posture
        self.orient = orientation
